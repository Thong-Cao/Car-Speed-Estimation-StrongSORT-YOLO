...
for name, model in models.items():
    ...
    X_train_t = RFE_model.transform(X_train_scaled)
    X_test_t = RFE_model.transform(X_test_scaled)
    print('Start training' + name)

    model.fit(X_train_t, y_train) 
    print(name + " trained.")

    accuracy, precision, recall, f1_score, report = evaluateModel(model, X_test_t, y_test) 
    results[name] = {
        'Accuracy': accuracy,
...
results_df.to_excel("./result_RFE.xlsx")
